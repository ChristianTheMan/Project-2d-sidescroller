...
'e' || key == '2' && rockPickedUp == true ) {
 ...
 for (int i = 0; i < 15; i++) { // Denne funktion er hvad der får
//stenene til at rykke sig fremad
 speedX = 0.1 + i ; // samt at indskrænke hvor langt
//stenen må flyve. plus at unequipped
 if (key == ' ' ) { // stenen så den ikke bare
//teleporter tilbage til playeren
 rockEquipped = false;
 rockPickedUp = false;
 x = x + speedX;
 }
